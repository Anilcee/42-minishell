	...
	int	execute_command(t_command *cmds, t_token *tokens, t_shell *shell)
{
	...
	// has_pipe, pipe komutlarının başlangıcıdır.
	if (has_pipe(cmds))
	{
		// ÇAĞRI GÜNCELLENDİ: Artık pipe executor'a her şeyi veriyoruz
		// ki child prosesler belleklerini temizleyebilsin.
		shell->last_exit_code = execute_piped_commands(cmds, tokens, shell);
		return (1);
	}

	// === Pipe olmayan tekli komutlar için mantık aynı kalır ===
	...
	{
		if (execute_builtin(cmds, shell) == -1) // exit komutu
		{
			restore_redirections(saved_stdout, saved_stdin);
			return (0); // Ana döngüye çıkış talebi
		}
	}
	else
	...
	int	process_input(char *input, t_shell *shell)
{
	...
	{
		printf("exit\n");
		return (0); // Döngüyü sonlandır
	}
	handle_signal_interrupt(shell);
	if (input && *input)
		add_history(input); // Readline'ın geçmişine ekle
	...
	if (cmds)
	{
		// ÇAĞRI GÜNCELLENDİ: execute_command'a token listesini de iletiyoruz.
		result = execute_command(cmds, tokens, shell);
	}
	else
	{
		// Parse hatası veya boş komut
		result = 1;
		if (g_signal_received == SIGINT)
			shell->last_exit_code = 130;
	}

	// Bu temizlik SADECE ANA (PARENT) PROSES içindir.
	// Child prosesler kendi temizliklerini `cleanup_and_exit` ile yapacak.
	free_tokens(tokens);
	free_commands(cmds);
	free(input);
	return (result);
